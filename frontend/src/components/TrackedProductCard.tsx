...
import { TrackedProduct, randomizePrice } from '../services/trackingService';
...
interface TrackedProductCardProps {
   ...
   */
  onUpdateThreshold?: (productId: string, newThreshold: number) => Promise<void>;
  ...
  /**
   * Callback when price is randomized (for testing)
   */
  onPriceRandomized?: (productId: string, newPrice: number) => void;
...
export default function TrackedProductCard({
  ...
  /**
   * Handle price randomization (for testing)
   */
  const handleRandomizePrice = async () => {
    ...
    try {
      ...
      if (onPriceRandomized) {
        onPriceRandomized(product.id, result.newPrice);
      }
      
      // In a real app, this would trigger a re-fetch of the product
      console.log('Price randomized:', result);
    } catch (error) {
      console.error('Failed to randomize price:', error);
    } finally {
  ...
  return (
    <div className="modern-card overflow-hidden">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between mb-2">
              ...
              <div className="flex items-center space-x-2 ml-4">
                <button
                  onClick={handleRandomizePrice}
                  disabled={isRandomizing}
                  className="text-white/40 hover:text-purple-400 transition-fast disabled:opacity-50"
                  title="Randomize price (for testing)"
                >
                  {isRandomizing ? (
      ...
      {showNotification && (
        <div className="fixed top-4 right-4 z-50 modern-card p-4 border-green-500/20 animate-slide-in">
